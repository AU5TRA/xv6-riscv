...
\begin{frame}
  ...
  raised the fault rate to a realistic level --- and the trace ring immediately
  could not keep up.
  ...
  \begin{table}\small
    \begin{tabular}{@{}ll@{}}
      \toprule
      Emission rate, paging workload & \ca 12{,}000 records/s \\
      Drain rate, \texttt{vmdrain} to the xv6 filesystem & \ca 5{,}300 records/s \\
      Ring capacity / time to overflow & 65{,}536 records / \ca 10\,s \\
      \midrule
      Measured & 302{,}243 emitted, \alert{142{,}629 dropped (47\%)} \\
      \bottomrule
    \end{tabular}
  \end{table}

  \vspace{-1mm}
  \textbf{This is a finding, not a gap.} Phase 0's gate explicitly allowed for
  it: if the packed record still cannot keep up, that is itself a result about
  the cost of kernel-resident instrumentation. The matrix now contains
  \emph{both} outcomes on purpose --- one lossless capture, and one that
  \good{must be rejected}, which is positive evidence that the drop discipline
  fires.

  \smallskip
  \textbf{Three remedies, costed:} pace the workload (changes nothing about the
  system under test); enlarge the ring to 32\,MiB (shifts every absolute page
  count); or stop emitting I/O span markers (loses I/O latency).
\end{frame}
